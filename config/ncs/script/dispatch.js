...
actions.ncsDispatchOrderRecommand = function($, namspace, request, response) {
  ...
  while (it.hasNext()) {
    var row = it.next()
    $.doWithoutTransaction(function(status) {
...
actions.ncsSigntimeRecommand = function($, namspace, request, response) {
  ...
  while (it.hasNext()) {
    ...
    var sql1 = "UPDATE NCS_ORDER_MOVEMENT_3DLOAD  OM3D SET OM3D.SHIP_END_TIME_SIGN = 0  WHERE OM3D.ORDER_MOVEMENT_GID in (select om.order_movement_gid from order_movement om  where om.SHIP_WITH_GROUP= ?)"
    ...
    if (row.adjust_time_sign == null || row.adjust_time_sign == undefined || row.adjust_time_sign == '') {
      var sql = "UPDATE NCS_ORDER_MOVEMENT_3DLOAD  OM3D SET OM3D.SHIP_END_TIME_SIGN = 1,OM3D.ADJUST_TIME=(select om.late_delivery_date from order_movement om where om.order_movement_gid=OM3D.order_movement_gid)+1/3 WHERE OM3D.ORDER_MOVEMENT_GID = ?"
      sql = sql.replaceAll("\"", "\'") //将双引号变成单引号2
      jdbcTemplate.update(sql, common.objectArray(row.order_movement_gid))

    } else {
      var sign = row.adjust_time_sign.toString().substring(0,10)+' '+ row.adjust_time_sign.toString().substring(11,16)
...
"  FROM order_movement om WHERE om.order_movement_gid = OM3D.order_movement_gid ),'YYYY-MM-DD') || LPAD('" + sign + "',4,'0'),'YYYY-MM-DD HH24MI') WHERE OM3D.ORDER_MOVEMENT_GID =?"*/
    var sql2 = "UPDATE NCS_ORDER_MOVEMENT_3DLOAD OM3D  "+
    "   SET OM3D.SHIP_END_TIME_SIGN = 1, OM3D.ADJUST_TIME = TO_DATE ('" + sign + "','YYYY-MM-DD HH24:MI:SS') WHERE OM3D.ORDER_MOVEMENT_GID =?"
...
actions.ncsRenewLoadByHandRecommand = function($, namspace, request, response) {
  ...
  var rows = request.body.selectedRows
  var trans_mode = rows[0].trans_mode_type
  var it = rows.iterator()
  var jdbcTemplate = $.queryService.jdbcTemplate
  while (it.hasNext()) {
    var row = it.next()
  ...
  jdbcTemplate.execute(
    new CallableStatementCreator({
      createCallableStatement: function(con) {
        ...
        }else if(trans_mode == '近地化入集配' || trans_mode == '干线入集配'){
          cs = con.prepareCall('{call  glogowner.ncs_plan_logic_pkg.ncs_om_ref_rule_with_oms_dc(?,?)}')
        }else if(trans_mode == '近地化出集配' || trans_mode == '干线出集配'){
          cs = con.prepareCall('{call  glogowner.ncs_plan_logic_pkg.ncs_om_ref_rule_with_oms_outdc(?,?)}')
        }
...
actions.ncsGenerateShipmentRecommand = function($, namspace, request, response) {
  ...
  for(var i = 0; i < rows.size(); i++){
    if(rows.get(i).get("manual_sign") == "Y"){
      count_num += 1
    }
  }

  //判断计数器的值和选中的行数是否相同，如相同表示是手工选单，不需要去check，把flag设置成1
  flag = parseInt(count_num) == parseInt(rows.size())?1:flag
...
actions.ncsOmChangeShipment = function($, namespace, request, response) {
  ...
  var to = aux.to
  var flag = 0
  var selection_l = aux.selection_l
  var rows_l = aux.rows_l
  if(rows_l != selection_l){
    throw "请选中目标区中的所有数据，再进行订单调整"
  }
...
actions.ncsRecalculateRecommand = function($, namspace, request, response) {
  ...
  //   var list  = $.queryService.select('ncs_v_ship_om')
  //                       .eq('shipment_gid',rows[0].shipment_gid)
  //                       .findAll()
  //   for(var i=0;i<list.size();i++){
  //
  //     if(list[i].length_sign == 0 && list[i].with_sign == 0 && list[i].high_sign == 0 ){
  //       flag = 1
  //       break
  //     }
  //   }
