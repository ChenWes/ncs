...
common.SCMP_INBOUNDS['CAR_APPLY'] = function($, reqHeader, reqBody, contentObj, logObj) {
  ...
  while (it.hasNext()) { // HEADER loop
    ...
    while (tdIt.hasNext()) { // TRAY_DETAILS loop 循环每一个托盘，把第三级的第一个明细放入第二级
      ...
      if (boxDetails && boxDetails.size() > 0) {
        ...
        var duplicat_all_part_code=''//存所有的零件号
        while (bdIt.hasNext()) {
          var boxDetail = bdIt.next()
          var count_issue_code = boxDetail.get('ISSUE_CODE')
          var count_part_code = boxDetail.get('PART_CODE')
          var issue_code = boxDetail.get('ISSUE_CODE').asText()
          var part_code = boxDetail.get('PART_CODE').asText()
          var tag = isContains(all_issue_code, issue_code)
          var tag_part_code = isContains(all_part_code, part_code)
          // print("==========issue_codetest"+issue_code) //判断是否包含
          // print("=======isContains" + tag)
          // print("==========part_codetest"+part_code) //判断是否包含
          // print("=======isContains_part" + tag_part_code)
         if (!tag) {
            all_issue_code = all_issue_code + issue_code + ","
           }
        if(!tag_part_code){
          all_part_code = all_part_code + part_code + ","
        }
          //去除重复出现的发行号
        ...
        all_part_code = all_part_code.substring(0, all_part_code.length - 1) //去除最后一个逗号
  // print("==========all_issue_codetext"+all_issue_code)
  duplicate_all_issue_code=all_issue_code
        duplicat_all_part_code = all_part_code
        tray.put('ALL_ISSUE_CODES', all_issue_code) // 将拼好的发行号放入第二级
        tray.put('ALL_PART_CODES', all_part_code)
  ...
  // print("==========all_issue_codetext111"+all_issue_code)
    ...
    while (tgIt.hasNext()) {
      ...
      var a_part_code = details.get('ALL_PART_CODES')
      ...
      header.put('ALL_PART_CODES', a_part_code)
      ...
      header.put('S_SUM',s_sum)
      header.put('F_SUM',f_sum)
    ...
    while (tgIt.hasNext()) {
      ...
      var arri_plan_date='';
        ...
        if (i == 0) {
          ...
          arri_plan_date = boxDetails[0].ARRI_PLAN_DATE
      ...
      header.put('BOX_QTY', box_qty)
      header.put('ARRI_PLAN_DATE', arri_plan_date)
      print("+++++++++b" + box_qty)
      box_qty = 0
