...
common.SCMP_ENV = {
  DEBUG: false,
  SCMP_URL: 'https://scmp-3pl.dongfeng-nissan.com.cn/MainService.asmx/RequestHandler', // 测试环境指向本地URL，后期可以配置到首选项
  OTM_URL: 'http://119.23.113.80:7778/GC3/glog.integration.servlet.WMServlet',
  OTM_DOMAIN: 'NCS'
}
...
tasks.scmpOutboundTask = function($, taskContext, args) {
  ...
  try {
    $.doInTransactionWithoutResult(function() {
      ...
      // print("+++++++++sql"+sql)
      //print("===========args"+args)
      //输出之间把send_flag设为1 Add by luo 20180505
      try {
          var i
          for (i = 0; i < rows.length; i++) {
            var rid=rows[i].id
            var send_sql = "update t01_scmp_api_log api set api.send_tag=1 where api.id=?"
            $.jdbcTemplate.update(send_sql, common.objectArray(rid))
          }
        }catch(e){
          e = common.throwable(e)
          e.printStackTrace()
